...
def generate_debug_imgs(anchors_a, scene_img_a, masks_a, y_pred,
                        step, obj_id_to_urdf, img_output_dir):
    # debug some reference examples
    #img_output_dir = os.path.join(opts.run_dir, 'debug_imgs')
    create_dir_if_required(img_output_dir)
    for obj_idx in range(len(masks_a)):
        urdf_id = obj_id_to_urdf[obj_idx]
        ...
        c.save(f"{img_output_dir}/step_{step:06d}.urdf_{urdf_id}.anchors.png")
        y_true_m = mask_to_pil_img(masks_a[obj_idx])
        y_pred_m = mask_to_pil_img(y_pred[obj_idx])
        c = collage([y_true_m, y_pred_m], rows=1, cols=2)
        c.save(f"{img_output_dir}/step_{step:06d}.urdf_{urdf_id}.y_true_pred.mask.png")
        ...
        c.save(f"{img_output_dir}/step_{step:06d}.urdf_{urdf_id}.y_true_pred.alpha.png")